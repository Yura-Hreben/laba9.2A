...
void Create(Student* p, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << " інформатика : "; cin >> p[i].markInfo;
		cout << endl;
...
void Print(Student* p, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << "| " << setw(17) << left << p[i].prizv
			...
			<< "| " << setw(24) << left << specStr[p[i].spec] << "| " << setw(7) << right << p[i].markPh << "| " << setw(11) << right << p[i].markMath << "|" << setw(15) << right << setw(13) << right << p[i].markInfo << "|" << endl;
...
double LineSearch(Student* p, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		{
			n++;
			if (p[i].markMath == 5 && p[i].markPh == 5 && p[i].markInfo == 5)
			{
				k++;
			}
			if (p[i].markPh == 5) {
...
void PrintIndexSorted(Student* p, int* I, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << "| " << setw(17) << left << p[I[i]].prizv
			...
			<< "| " << setw(24) << left << specStr[p[i].spec] << "| " << setw(7) << right << p[I[i]].markPh << "| " << setw(11) << right << p[I[i]].markMath << "|" << setw(15) << right << setw(13) << right << p[I[i]].markInfo << "|" << endl;
